...
async def on_ready():  # print in the console when bot wake up
    ...
    await bot.change_presence(activity=discord.Game("cartonner tes morts"))

def pourcentage():
    nowPreciseDate = datetime.now()  # get exact date with seconds
    toPourcent = (int(nowPreciseDate.strftime("%H"))+ 1) * 60 + int(nowPreciseDate.strftime("%M"))
    if toPourcent <= 480:
        return str(0)
...
def sendHoraire(ctx, minute, hour, current_day):
    ...
    if current_day > 4:
        ...
        if period != 0:
            period -= 1
    print("Jour:", current_day, "Cours:", period, "Heure", hour, "Minutes:", minute, "Classe:",
        ...
        if current_day == jour:  # this way i use the day i got as an useful variable
            ...
            for heurePeriod in range(8 - period):  # because i only want the bot to print today's remaining courses
                ...
                toPrint += ("\n" + str(heurePeriod + period + 1) + "h " + schoolTimetableArray[schoolRole][current_day][
                    period + theLeftOfUs])
            toPrint += ("\nOn est à " + pourcentage() + "% de la journée scolaire et " + str(
                round((((int(preciseDate.strftime("%H"))+1) * 60 )+ (int(preciseDate.strftime("%M")))) / 1440 * 100, 1)) + "% de la journée```")
            return toPrint
